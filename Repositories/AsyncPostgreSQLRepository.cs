...
{
    /// <summary>
    /// Async PostgreSQL repository with native async database operations and bulk support.
    /// Uses AsyncPostgreSQLStore which provides bulk operations via transactions.
    /// </summary>
    /// <typeparam name="TViewModel">The type of view model.</typeparam>
    /// <typeparam name="TModel">The type of data model.</typeparam>
    public abstract class AsyncPostgreSQLRepository<TViewModel, TModel>
        ...
        where TViewModel : Models.ILoadable<TModel>
    {
        ...
        /// </summary>
        public PostgreSQLConnector? Connector => Store?.GetUnwrappedStore<TModel, AsyncPostgreSQLStore<TModel>>()?.Connector;

        /// <summary>
        /// Initializes a new instance of the AsyncPostgreSQLRepository class.
        /// </summary>
        public AsyncPostgreSQLRepository()
            : base(null)
        {
            Store = new AsyncPostgreSQLStore<TModel>();
        }

        /// <summary>
        /// Initializes a new instance with dependency injection support.
        /// </summary>
        /// <param name="store">The async PostgreSQL store to use (optional). Can be wrapped (e.g., by tenant wrappers).</param>
        public AsyncPostgreSQLRepository(Data.Stores.IAsyncStore<TModel>? store)
            : base(null)
        {
            if (store != null && !store.IsStoreOfType<TModel, AsyncPostgreSQLStore<TModel>>())
            {
                throw new ArgumentException(
                    "Store must be of type AsyncPostgreSQLStore<TModel> or a wrapper around it (e.g., AsyncTenantStoreWrapper).",
                    nameof(store));
            }
            Store = store ?? new AsyncPostgreSQLStore<TModel>();
        }
        ...
        public void SetSettings(RemoteSettings settings)
        {
            if (settings != null)
            {
                var innerStore = Store?.GetUnwrappedStore<TModel, AsyncPostgreSQLStore<TModel>>();
